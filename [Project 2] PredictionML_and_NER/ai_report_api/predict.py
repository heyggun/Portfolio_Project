...
from utils.model.model4_feature import Model4
from utils.config.common import *
from utils.functions import *
class predictScore(Model1, Model2, Model3, Model4) :
    def __init__(self):
        super(predictScore, self).__init__()

    def get_model_scores(self, df, ptr_sex):

        # df = self.to_numeric(df)
        # model_1_df, model_2_df, model_3_df = self.data_splitter(df)
        model_1_df, model_2_df, model_3_df, model_4_df = self.data_splitter(df)
        ...
        model_4_df = self.model_4_transform(model_4_df)
        ...
        if ptr_sex == "f":
            model_ner_yn = self.get_diversity_yn(model_4_df['ptr_ners'][0])
            model4_keywords = model_4_df['ptr_keywords'][0]

        else:
            model_ner_yn = self.get_diversity_yn(model_4_df['ners'][0])
            model4_keywords = model_4_df['keywords'][0]

        total_score = round(((model_1_score * 0.2) + (model_2_score * 0.3) + (model_3_score * 0.5)), 1)
        return model_1_score, model_2_score, model_3_score, total_score, model_ner_yn, model4_keywords
    ...
    def data_splitter(self, df):
        ...
        model_4_df = df[['mate_conts','ptr_mate_conts']]
    ...
    def get_diversity_yn(self, ners):
        ner_cnt = len(ners)
        if ner_cnt >=2:
            result = 'y'
        else:
