...
# base data
if str(conf.port).startswith('80') :
    member_mate = pd.read_csv(f'{dataPath}/member_mate.csv')
else :
    member_mate = pd.read_csv(f'{dataPath}/member_mate_dev.csv')

member_mate = member_mate[['mem_no', 'mem_sex', 'mem_birth_year', 'mem_loc', 'mate_ann_salary',
                           'mate_charc', 'smoke_slct', 'drink_slct', 'health_slct',
                           'wed_plan', 'mate_slct', 'divorce_year', 'parents_slct',
                           'mate_religion','mate_hobby', 'favor_food','mate_job',
                           'mate_career', 'mate_height','mate_weight', 'mate_car','possess_property']]
male_df = member_mate[member_mate['mem_sex']=='m']
female_df = member_mate[member_mate['mem_sex']=='f'].drop('smoke_slct', axis=1)
female_df.columns = ['ptr_' + i for i in female_df.columns]
...
def df_merge(my_info, mem_no, sex) :
    if sex == 'm' :
        my_info = my_info.rename(columns=column_keys)
    else :
        my_info = my_info.rename(columns=column_keys_f)
    if sex == 'm' :
        my_birth = my_info['mem_birth_year'].values[0]
        range_index = female_df[abs(int(my_birth) - female_df['ptr_mem_birth_year'] <= 5)].index
        range_df = female_df.loc[range_index]
        range_df['mem_no'] = mem_no
    else :
        my_birth = my_info['ptr_mem_birth_year'].values[0]
        range_index = male_df[abs(int(my_birth) - male_df['mem_birth_year'] <= 5)].index
        range_df = male_df.loc[range_index]
        range_df['ptr_mem_no'] = mem_no
    df = pd.merge(my_info, range_df)
    if len(df) > 2000 :
        df = df.sample(2000)
    df['mem_no'] = df['mem_no'].astype(int)
    df['ptr_mem_no'] = df['ptr_mem_no'].astype(int)
    return df

def detect_model_type(search_type) :
    if search_type == 'g':
        search_type = 'gscore'
    elif search_type == 'c':
        search_type = 'cscore'
